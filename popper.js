async function getdatafromapi(moviename) {
  try {
    var res = await fetch(
      `https://api.themoviedb.org/3/search/movie?api_key=d4aca09bd1a95ab9dba949e473a0abf4&language=en-US&query=${moviename}&page=1&include_adult=false`
    );
    ...
    if (MovieData) {
      var Movie_Cast_raw = await fetch(
        "https://api.themoviedb.org/3/movie/" +
          String(MovieData.id) +
          "/credits?api_key=d4aca09bd1a95ab9dba949e473a0abf4&language=en-US"
      );
      Movie_Cast = await Movie_Cast_raw.json();

      var Movie_metadata_raw = await fetch(
        `https://api.themoviedb.org/3/movie/${MovieData.id}?api_key=d4aca09bd1a95ab9dba949e473a0abf4&language=en-US`
      );
...
document.addEventListener("DOMContentLoaded", () => {
  document.getElementById("getmoviebutton").addEventListener("click", () => {
    var moviename = document.getElementById("moviename-input").value.trim();
    if (moviename) {
